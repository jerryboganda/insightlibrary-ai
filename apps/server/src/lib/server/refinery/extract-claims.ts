...
import { chunks as chunksTable, claims, claimSources, topics } from '../db/schema';
...
export async function extractClaimsForDocument(
	...
	const cap = Number(process.env.CLAIMS_MAX_CHUNKS ?? '60');
	let extracted = 0;
	...
	for (let i = 0; i < Math.min(rows.length, cap); i++) {
		...
		for (let j = 0; j < res.claims.length; j++) {
			...
			const emb = await embedText(text).catch(() => null);
			await db
				.insert(claims)
				.values({
					id: claimId,
			...
			await db
				.insert(claimSources)
				.values({ id: `cs_${claimId}`, claimId, documentId, chunkId: chunk.id, blockId: chunk.blockId ?? null, stance: 'supports' })
				.onConflictDoNothing();
			extracted++;
		}
	}
